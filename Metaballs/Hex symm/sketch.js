...
class MetaBallRender{
    ...
    getCol(x,y){
        ...
        for(let ball of this.balls){
            va += (ball.mult*( ball.r/dist(x,y,ball.x,ball.y)) )
            va += ( ball.mult*(ball.r/ dist(x,y, width-ball.x, ball.y)))
        }
        ...
        if(va < 0.1 + 0.1*noise(frameCount*0.001, p1*0.1, y*0.1) ) va = 0 
        else va = 1

        // Use cpickFn for continuous spectrum
