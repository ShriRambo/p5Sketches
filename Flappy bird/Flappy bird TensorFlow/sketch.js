...
function draw(){
    ...
    for(let n= 0; n< slider.value();n++){
        ...
        for (jj= birds.length -1; jj >= 0; jj--){
            if(birds[jj].bottomTop()) {
                savedBirds.push(birds.splice(jj,1)[0]);
            }
        }
...
class Bird{
    ...
    update() {
        ...
        this.score += (1 + Math.exp(-0.01*abs(this.v)    )) ;
    ...
    bottomTop() {
        ...
        return (this.y >= height - 0.5*this.d || this.y <= 0.5*this.d);
      }
